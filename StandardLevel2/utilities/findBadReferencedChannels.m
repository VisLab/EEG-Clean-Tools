function referenceOut = findBadReferencedChannels(signal, referenceIn)
% Find bad channels to be interpolated with respect to referenced signal

%% Check the input parameters
if nargin < 1
    error('findBadReferencedChannels:NotEnoughArguments', 'requires at least 1 argument');
elseif isstruct(signal) && ~isfield(signal, 'data')
    error('findBadReferencedChannels:NoDataField', 'requires a structure data field');
elseif size(signal.data, 3) ~= 1
    error('findBadReferencedChannels:DataNotContinuous', 'signal data must be a 2D array');
elseif size(signal.data, 2) < 2
    error('findBadReferencedChannels:NoData', 'signal data must have multiple points');
elseif ~exist('referenceIn', 'var') || isempty(referenceIn)
    referenceIn = struct();
end
if ~isstruct(referenceIn)
    error('findBadReferencedChannels:NoData', 'second argument must be a structure')
end
