function showCollectionStatistics(collectionStats, legendString, collectionColors)
% Displays the items in collectionStats using boxplots

%% Show median window correlation versus median window deviation
titles = legendString;
...
for k = 1:length(dataDev)
    plot(dataCorr{k}, dataDev{k}, 'Color', collectionColors(k, :), ...
        'Marker', 'o', 'LineStyle', 'none')
end
hold off
set(gca, 'FontSize', 12)
xlabel('Mean min win channel correlation', 'FontSize', 12)
ylabel('SDR/Median win channel deviation', 'FontSize', 12)
title(baseTitle)
